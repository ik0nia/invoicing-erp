<?php
    ...
    $statusText = 'In asteptarea completarii datelor.';
    if ($mode === 'portal') {
        $statusText = 'Acces activ la documente si contracte.';
    } elseif ($confirmedAt) {
        $statusText = 'Inrolare finalizata. Puteti continua cu documentele.';
    }
?>
...
<div class="max-w-5xl">
    ...
    <?php if (!empty($permissions['can_view'])): ?>
        ...
        <div class="mt-6 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
                        ...
                        <tbody>
                            <?php foreach ($contracts as $contract): ?>
                                <?php
                                    ...
                                    $previewUrl = App\Support\Url::to('p/' . $token . '?preview=' . (int) $contract['id'] . '#preview');
                                ...
                                <tr class="border-t border-slate-100">
                                    ...
                                    <td class="px-3 py-2 text-slate-600">
                                        <div class="flex flex-wrap gap-3 text-xs font-semibold">
                                            <a href="<?= htmlspecialchars($previewUrl) ?>" class="text-blue-700 hover:text-blue-800">Previzualizeaza</a>
                                            <?php if ($hasGenerated): ?>
            ...
            <?php if ($previewHtml !== '' && $previewContract): ?>
                <div class="mt-6" id="preview">
                    <div class="text-sm font-semibold text-slate-700">Previzualizare: <?= htmlspecialchars((string) ($previewContract['title'] ?? 'Contract')) ?></div>
