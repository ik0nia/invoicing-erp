        ...
        <?php if ($data): ?>
            ...
            <div class="grid">
                ...
                <div>
                    ...
                    <div class="muted">Data: <?= htmlspecialchars($formatDate($data['issue_date'] ?? null)) ?></div>
                    <div class="muted">Scadenta: <?= htmlspecialchars($formatDate($data['due_date'] ?? null)) ?></div>
                </div>
            </div>
            ...
            <div class="totals">
                <div>
                    ...
                    <div class="value"><?= htmlspecialchars($formatMoney($data['total_without_vat'] ?? 0, $currency)) ?></div>
                </div>
                <div>
                    <div class="label">TVA</div>
                    <div class="value"><?= htmlspecialchars($formatMoney($data['total_vat'] ?? 0, $currency)) ?></div>
                </div>
                <div>
                    <div class="label">Total cu TVA</div>
                    <div class="value"><?= htmlspecialchars($formatMoney($data['total_with_vat'] ?? 0, $currency)) ?></div>
                </div>
            </div>
                    ...
                    <?php else: ?>
                        <?php foreach ($lines as $index => $line): ?>
                            <tr>
                                ...
                                <td class="right"><?= htmlspecialchars($formatMoney($line['unit_price'] ?? 0, $currency)) ?></td>
                                <td class="right"><?= htmlspecialchars(number_format((float) ($line['tax_percent'] ?? 0), 2, '.', ' ')) ?>%</td>
                                <td class="right"><?= htmlspecialchars($formatMoney($line['line_total_vat'] ?? 0, $currency)) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
